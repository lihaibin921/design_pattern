...
---
### 抽象工厂模式
>抽象工厂模式是工厂方法模式的进一步延伸,工厂类提供多个工厂方法生产多种不同类型的产品,形成产品族  
>抽象工厂模式新增一个产品族比较方便,不需要修改抽象工厂和抽象产品,新增具体工厂/产品类即可  
>在产品族中新增一类产品则比较麻烦,需要对四类角色均进行修改/新增  
>适用于含有多个产品族,且系统稳定后不新增产品类型的情况  
#### 抽象工厂模式的角色
>AbstractFactory:抽象工厂,声明创建一族产品方法,每个方法对应一种抽象产品  
>ConcreteFactory:具体工厂,实现抽象工厂,生成一族具体产品  
>AbstractProduct:抽象产品,声明具体产品的业务,是抽象工厂创建的产品族中的一个产品  
>ConcreteProduct:具体产品,实现抽象产品
---
### 建造者模式
>将复杂对象的构建(组装过程)与它的表示(创建的对象)分离,相同的创建过程能创建不同的对象  
>客户端不必知道产品的细节,创建过程和对象本身被解耦和  
>复杂对象的创建过程有清晰的记录,更方便控制  
>**场景**:产品对象内部复杂,有多个成员属性,对象属性有生成顺序等  
#### 建造者模式中的角色
>Product:产品角色,被构建的复杂对象,包含多个成员属性  
>Builder:抽象构建者,接口/抽象类,声明Product各组构建方法.包括俩类方法buildXXX()
>为各组建构建方法,getResult()为返回复杂对象方法  
>ConcreteBuilder:具体构建者,实现builder接口  
>Director:指挥者,安排复杂对象的创建流程,使用builder接口创建product  
### 原型模式
>原型模式不多介绍了,以某个对象为原型进行复制,java中有深浅克隆或者序列化的方式实现  
## 结构型
...
### 享元模式
>通过共享细粒度对象,减少系统中对象数量  
>但享元对象的内部/外部状态较难理解,也增加了程序的复杂性  
>Integer -128~127都是享元模式的应用  
#### 享元模式中的角色
>Flyweight:抽象享元类,接口/抽象类,声明具体享元类的公共方法,这些方法可以对外提供内部状态,也可以用来设置外部状态  
>ConcreteFlyweight:具体享元类,实现抽象享元类,通常可以使用单例模式创建具体享元类对象,存放进享元池  
>UnsharedConcreteFlyweight:非共享具体享元类,实现抽象享元类,提供非共享的享元对象,可以直接创建,不用放入享元池  
>FlyweightFactory:享元工厂类,一般用静态工厂即可,创建一个享元池(一般是map),共享对象存入享元池,用户请求时没有则创建,
>有则直接从享元池取出
#### 单纯和复合
>单纯享元模式:只能有共享享元 , 不能存在非共享的  
>复合享元模式:单纯享元对象组合成复合享元对象,复合享元对象本身是非共享的,但可以分解成单纯享元对象,可以共享    
## 行为型
